---
title: "FC Metrics Susceptibility to Motion- Low-motion Edges"
output:
  html_document:
    theme: united
    toc: yes
...
for (i in 1:4){
  ...
  for (l in 1:6){
    assign("metric", metrics[l])
    temp <- data %>% dplyr::select(.,matches(paste0("_",metric,"$")),matches(paste0("^",metric,"$")),matches("motion"), matches("Age_numeric"),matches("Gender_numeric"))
    print(colnames(temp))
    temp2 <- cbind(temp[1], temp$Age_numeric, temp$Gender_numeric)
    pcorr_modul_estimate[l] <- as.numeric(pcor.test(temp[5], temp$motion, temp2, method = "kendall")$estimate)
  }
