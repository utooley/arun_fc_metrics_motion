...
p<-ggplot(all, aes(x=baseline, y=value, color=ranking)) + 
   geom_jitter(position=position_jitter(0.1), cex=1)
p + theme_classic() + stat_summary(fun.data="mean_sdl",fun.args = list(mult = 1) ,
                 geom="pointrange", shape=18, cex=1)+ labs(title="333-node Gordon parcellation",x="", y = "Correlation of Q with motion")+ theme(plot.title = element_text(hjust = 0.5))+ theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14)) +scale_color_manual(values=c("#0072BD","#77AC30"))
```
